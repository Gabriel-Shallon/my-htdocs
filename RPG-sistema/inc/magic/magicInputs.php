        ...
        (function() {
            ...
            function generateTargetInputs() {
                ...
                for (let i = 0; i < numTargets; i++) {
                    ...
                    let targetSelectHTML = `<label>Selecionar Alvo: <select id="magic_target" name="magic_targets[${i}][name]" required>`;
                    targetSelectHTML += '<?php selectTarget($caster, $validTargets); ?>';
                    targetSelectHTML += '</select></label>';
                    let reactionSelectHTML = `<br><label>Reação do Alvo: <select name="magic_targets[${i}][reaction]" required>`;
                    reactionSelectHTML += `<option value="defender">Defender</option>`;
                    reactionSelectHTML += `<option value="defender_esquiva_deflexao" style="display:none;">Deflexão (2PM)</option>`;
                    ...
                    fieldset.querySelector('.magic-target-select').dispatchEvent(new Event('change'));
                }
            }
...
function inputBrilhoExplosivo($caster, &$b){
    ...
    echo '<label>Selecionar Alvo: <select id="magic_target" name="magic_target" required>';
    selectTarget($caster, $validTargets);
    ...
    echo 'Reação: <select id="def" name="magic_def" class="magic-reaction-select">'
        ...
        . '<option value="defender_esquiva_deflexao" style="display:none;">Deflexão (2 PM)</option>'
        . '</select>';
...
function inputEnxameDeTrovoes($caster, &$b){
    $validTargets = getValidTargets($caster, $b);
    echo '<label>Selecionar Alvo: <select id="magic_target" name="target" required>';
    selectTarget($caster, $validTargets);
    echo '</select></label><br>';
